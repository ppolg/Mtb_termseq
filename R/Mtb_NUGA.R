...
#     - Export them (duh)
#   • Aaaand now with added uORF shenenigangs! Woo-hoo!!
...
#### Tine's Turbulent nt4 Test ####
# AKA what is tge distribution of 4th nt's in regards to NUGA?
...
# How many NUGA has a terminator? In all datasets?
...
ggplot(overlaps_percent,aes(x = n_overlap_next, y=n*100)) +
  ...
  xlab("Overlap length (nt)") +
  ylab("Frequency") +
  ggtitle("Annotated-annotated (frequency)") +
  scale_linetype_manual(values = c(4,1)) +
  scale_y_continuous(expand=c(0,0), limits=c(0,100), labels = c("0%","25%","50%","75%","100%")) +
  scale_x_continuous(limits = c(0,25), expand = c(0,0), breaks = c(1,4,7,10,13,16,19,22,25))
...
ggplot(uORF_main_percent,aes(x = n_overlap_main, y=n*100)) +
  ...
  xlab("Overlap length (nt)") +
  ylab("Frequency") +
  ggtitle("uORF-annotated (frequency)") +
  scale_linetype_manual(values = c(4,1)) +
  scale_y_continuous(expand=c(0,0), limits=c(0,100), labels = c("0%","25%","50%","75%","100%")) +
  scale_x_continuous(limits = c(0,25), expand = c(0,0), breaks = c(1,4,7,10,13,16,19,22,25))
...
fwrite(print_uORF_self, file = here::here("NUGA_Out/uORF_uORF_OL.csv"))
fwrite(print_uORF_main, file = here::here("NUGA_Out/uORF_annot_OL.csv"))
